 ...
 */
export const NFAArbitrary: fc.Arbitrary<NFA> = fc.record({
  ...
  return fc.record({acceptStates}).chain(({acceptStates})=> {
    ...
    const transitionsArb = fc.array(
      fc.tuple(
        ...
        fc.tuple(fc.constantFrom(...Array.from(alphabet)),fc.subarray(states,{ minLength: 0, maxLength: states.length }))
      ),
      { minLength: 1, maxLength: 10 }
    );

    // Optional ε transitions
    const epsilonTransitionsArb = fc.array(fc.tuple(fc.constantFrom(...states),fc.subarray(states, { maxLength: states.length })),{minLength : 0,maxLength : 10});

...
export class TMState extends State {
    ...
    public insertTransition(input: char, readTape: char, writeTape: char, move: Move, to: string): void {
        let bucket = this._transitions.get(input);

        if (!bucket) {
            bucket = new Set<TMEdge>();
            bucket.add({readTape, writeTape, move, to});
            this._transitions.set(input, bucket);
        } else if (!bucket.has({readTape, writeTape, move, to})) {
            bucket.add({readTape, writeTape, move, to});
        }
    }

    public removeTransition(input: char, readTape: char, writeTape: char, move: Move, to: string): boolean {
        let bucket = this._transitions.get(input);
        if (!bucket) return false;

        return bucket.delete({readTape, writeTape, move, to});
    }
